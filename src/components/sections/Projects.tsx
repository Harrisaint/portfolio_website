  ...
  return (
    ...
    >
      ...
      <div className="relative z-10">
        ...
        >
          <div className="inline-flex items-center space-x-2 px-4 py-2 mb-6 rounded-full glass-light border border-gray-200/50">
...
function ProjectCard({ project, index }: { project: Project; index: number }) {
  return (
    ...
    >
      ...
      <div className="relative h-full p-8 rounded-3xl glass-light border border-gray-200/50 hover:border-blue-500/50 transition-all shadow-lg hover:shadow-2xl">
        {/* Category Badge */}
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {project.technologies.slice(0, 4).map((tech) => (
            <span
              key={tech}
              className="px-3 py-1 text-xs rounded-lg bg-gray-100 text-gray-700 border border-gray-200"
            >
              {tech}
            </span>
          ))}
          {project.technologies.length > 4 && (
            <span className="px-3 py-1 text-xs rounded-lg bg-gray-100 text-gray-700 border border-gray-200">
              +{project.technologies.length - 4}
            </span>
          )}
        </div>
        ...
        <div className="flex items-center space-x-4 pt-4 border-t border-gray-200/50">
          {project.githubUrl && (
            <motion.a
              ...
              className="flex items-center space-x-2 text-gray-700 hover:text-blue-600 transition-colors group/link"
            >
